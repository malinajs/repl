...
(function () {
    ...
    */
    ...
    function e(e){return {subLanguage:"xml",contains:[e.COMMENT("\x3c!--","--\x3e",{relevance:10}),{begin:/^(\s*)(<script(\s*context="module")?>)/gm,end:/^(\s*)(<\/script>)/gm,subLanguage:"javascript",excludeBegin:!0,excludeEnd:!0,contains:[{begin:/^(\s*)(\$:)/gm,end:/(\s*)/gm,className:"keyword"}]},{begin:/^(\s*)(<style.*>)/gm,end:/^(\s*)(<\/style>)/gm,subLanguage:"css",excludeBegin:!0,excludeEnd:!0},{begin:/\{/gm,end:/\}/gm,subLanguage:"javascript",contains:[{begin:/[\{]/,end:/[\}]/,skip:!0},{begin:/([#:\/@])(if|else|each|await|then|catch|debug|html)/gm,className:"keyword",relevance:10}]}]}}function svelte(n){n.registerLanguage("svelte",e);}

    function Editor($element, $option) {
...
<style>
        ...
        tab-size: 4;
            ...
            if($option.afterElement) {
                ...
                $$build4($cd, $element);
                    ...
                    $component.setProp_oninput = (value) => {
                        if(oninput == value) return;
                        oninput = value;
                        $$apply();
                    };
    ...
    function frameCommunicator(iframe,callback){
        const frame_id = uid++;
...
`;
    ...
    function Result($element, $option) {
      if ($option == null) $option = {};
      let files;
      ...
      function compile() {
        ...
        cooldown_timer = setTimeout(async () => {
          $$apply();
            ...
            if($component) {
                if($component.destroy) $cd.d($component.destroy);
                    ...
                    if($component.setProp_oninput) {
                        $watch($cd, () => ((onEditorInput)), $component.setProp_oninput, {d: true, ro: true});
                    } else console.error("Component Editor doesn't have prop oninput");
            }
        }
    {
            ...
            if($component) {
                if($component.destroy) $cd.d($component.destroy);
                    ...
                    } else console.error("Component Result doesn't have prop files");
            }
        }}        const rootTemplate = `
